...
const CanceledJobs = () => {
    ...
    const fetchCanceledJobs = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/jobs/canceled`, {
                method: 'GET',
                headers: {
            ...
            if (!response.ok) {
                throw new Error('Failed to fetch canceled jobs');
            }

            const data = await response.json();
            return data.jobs;
        } catch (error) {
            console.error('Error fetching canceled jobs:', error);
            return [];
        }
    };
